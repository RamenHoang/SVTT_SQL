    ...
    success: function (res) {
      ...
      for (const ngay in data) {
        if (data.hasOwnProperty(ngay)) {
          ...
          html +=
            '<div class="time-label"><span class="bg-success" id="ngay">' +
            ngay +
            "</span></div>";
          ...
          data[ngay].forEach((item) => {
            html +=
              '<div><i class="fas fa-envelope bg-primary"></i><div class="timeline-item"><h3 class="timeline-header"><b id="tendetai">' +
              item.ten +
              " [" +
              item.tennhom +
              ']</b></h3><div class="timeline-body" id="mota">' +
              item.mota +
              '</div><div class="timeline-footer"></div></div></div>';
